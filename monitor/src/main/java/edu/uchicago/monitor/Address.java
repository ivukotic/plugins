...
public class Address {
	...
	Address(String param) throws IllegalArgumentException{
		...
		temp=param.split(":");
		if (temp.length<2)
			throw new IllegalArgumentException("Incorrect collector address format: "+param+" format should be <address>:<port>[:<delay>].");
		address=temp[0];
	...
	public String toString(){
		return "Hostname: "+address+"\t port: "+port+".\t Sending each "+delay+" seconds.\n";
	}
}
